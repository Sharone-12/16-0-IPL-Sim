// ===================== 16-0 — Season simulation (MULTIPLAYER) =====================
// Cloned from simulation.js. The league/playoff engine, results feed, leaders,
// table and detailed scorecards are reused VERBATIM. Only team construction and
// boot are rewired for multiplayer, plus a seeded RNG so every client in a room
// computes byte-identical results without any host coordination.

// ---- multiplayer hooks ----
const MP_SUPA =
  (typeof initSupabase === "function" && initSupabase()) ||
  (typeof supabaseClient !== "undefined" ? supabaseClient : null);
const MP_PID = sessionStorage.getItem("mp_pid");
const MP_ROOM = new URLSearchParams(location.search).get("room");
const MP = !!MP_ROOM;

// Seed Math.random (mulberry32) from the room id so all clients share one RNG
// stream. Must be called before any simulation runs.
function mpSeedRng(str) {
...
// ---- multiplayer boot: build all 10 teams from drafted XIs in Supabase ----
async function mpBoot() {
  ...
  try {
    ...
    // Local player's drafted team is "Your XI"; everyone else is an opponent.
    USER_ID = MP_PID;
...
function buildMpTeams(rows) {
  return rows.map((pr) => {
    ...
    const name = pr.username || pr.name || pr.id;
    return {
      id: pr.id,
      name,
...
function initSeason() {
  if (MP) {
    // 10 real drafted teams. Derive groups from a STABLE id sort (identical on
    // every client) so fixtures + the seeded sim match across the room...
    state.teams = state.__mpTeams;
    const stableIds = state.teams.map((t) => t.id).sort();
    GROUPS.A = stableIds.slice(0, 5);
    GROUPS.B = stableIds.slice(5, 10);
    // ...then float the local player's team to teams[0] for the "Your XI" views.
...
async function submitToLeaderboard(stageStr = "Unknown") {
  if (MP) return null; // MP seasons don't post to the solo leaderboard
...
function saveCompletedSeasonState(completedData) {
  if (MP) return; // no localStorage season persistence in MP
  try {
    const saved = JSON.parse(localStorage.getItem("seasonState") || "{}");
